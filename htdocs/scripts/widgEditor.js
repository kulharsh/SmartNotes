...
function widgEditor(replacedTextareaID)
{
	...
	this.loadArea = document.createElement("span");
	this.loadArea.id = "load";
	
	this.saveButton = document.createElement("button");
	var t = document.createTextNode("Save");       // Create a text node
	this.saveButton.appendChild(t);
	this.saveButton.id="Save";
	this.saveButton.onclick = function(){self.saveDoc(self);return false;};
	
	this.newButton = document.createElement("button");
	var t = document.createTextNode("New");       // Create a text node
	this.newButton.appendChild(t);
	this.newButton.id="New";
	this.newButton.onclick = function(){self.newDoc(self);return false;};
	
	this.loadArea.innerHTML = "<p> This is the load area</p>"
	this.IE = false;
	this.locked = true;
	this.pasteCache = "";
	this.wysiwyg = true;
	this.SolrID = String(new Date().getTime());
	this.modifiedFunctions = false;
	if (document.all)
	{
		this.IE = true;
	}
	...
	this.theContainer.appendChild(this.loadArea);
	this.theContainer.appendChild(this.saveButton);
	this.theContainer.appendChild(this.newButton);
...
widgEditor.prototype.initEdit = function()
{
	...
	if (typeof document.addEventListener == "function")
	{
		...
		this.theIframe.contentWindow.document.addEventListener("keyup", function(event){widgToolbarCheckState(self); keyPressedIntextarea(event,self);return true;}, false);
...
function keyPressedIntextarea(event, theWidgEditor)
{
   document.getElementById("save").innerHTML = "Changes Unsaved"; 	
   if(event.keyCode == 13)
   {
	   //alert('key pressed');
		theWidgEditor.updateWidgInput();
  ...
  //alert(theWidgEditor.theInput.value);
  var x = String(theWidgEditor.theInput.value);
  //alert(x);
  var url = "/smNotes/cgi-bin/search.py?q=" + x;
  //alert(url)
...
function loadDoc(widgEditorObject, id) 
{	
	document.getElementById("save").innerHTML = "Loading..";
	document.getElementById("result").innerHTML = "Related documents...";
	
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() 
		{
			if (xhttp.readyState == 4 && xhttp.status == 200) {
				document.getElementById("save").innerHTML = 'Load Complete';
				widgEditorObject.theTextarea.value = xhttp.responseText;
				widgEditorObject.theInput.value = widgEditorObject.theTextarea.value;
				widgEditorObject.writeDocument(widgEditorObject.theInput.value);
				widgEditorObject.SolrID = id;
				widgEditorObject.initEdit()
		}
		else{
			document.getElementById("save").innerHTML = "Loading...";
		}
  ...
  //alert(theWidgEditor.theInput.value);
  var x = String(widgEditorObject.theInput.value);
  //alert(x);
  var url = "/smNotes/cgi-bin/loadDoc.py?id=" + String(id);
  //alert(url)
  xhttp.open("GET", url, true);
  xhttp.send();
 
}
widgEditor.prototype.addSave = function()
{
	//alert(this);
	var self = this;
	//document.getElementById("SaveButton").onclick = function(){saveDoc(self);};
	//document.getElementById("NewButton").onclick = function(){newEditor(self);};
	
}
widgEditor.prototype.saveDoc = function(widgEditorObject)
{
	//alert(widgEditorObject);
	//alert('call aya!');
	var xhttp = new XMLHttpRequest();
	widgEditorObject.updateWidgInput();
	xhttp.onreadystatechange = function() 
	{
		//alert(document.getElementById("save"));
		if (xhttp.readyState == 4 && xhttp.status == 200) {
			//alert(xhttp.responseText);
			document.getElementById("save").innerHTML = xhttp.responseText;
			widgEditorObject.loadList();
		}
		else{
			document.getElementById("save").innerHTML = "Saving..";
		}
	};
	var q = String(widgEditorObject.theInput.value);
	var id = String(widgEditorObject.SolrID);
	var url = "/smNotes/cgi-bin/save.py?q=" + q + "&id=" + id;
	//alert(url);
	xhttp.open("GET", url, true);
	xhttp.send();
}

widgEditor.prototype.newDoc = function(widgEditorObject)
{
	widgEditorObject.SolrID = String(new Date().getTime());
	widgEditorObject.theTextarea.value="Type the text here!";
	widgEditorObject.theInput.value = widgEditorObject.theTextarea.value;
	widgEditorObject.writeDocument(widgEditorObject.theInput.value);
	document.getElementById("save").innerHTML = "Changes unsaved";
	document.getElementById("result").innerHTML = "Related documents";
...
widgEditor.prototype.loadList = function()
{
	var self = this;
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() 
	{
		if (xhttp.readyState == 4 && xhttp.status == 200) {
		self.loadArea.innerHTML = xhttp.responseText;
		var links = self.loadArea.getElementsByClassName("loadClass");
		for (i = 0; i < links.length; i++) { 
			self.modifiedFunctions=true;
			links[i].onclick = function(){loadDoc(self,this.id);};
		}
		}
		else{
			self.loadArea.innerHTML = "Loading..";
		}
	};
	var url = "/smNotes/cgi-bin/loadList.py";
	//alert(url);
	xhttp.open("GET", url, true);
	xhttp.send();
